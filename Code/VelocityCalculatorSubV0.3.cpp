...
public:
  ...
  void counterCallback(const sensor_msgs::JointState::ConstPtr& msg) {
    ...
    //ROS_INFO("I counted: [%d]", msg->header.stamp.nsec);
      ...
      float w_ticks_fl= (dticks_fl/dtime) * ((2*M_PI)/(this->N * this->T));
      float w_ticks_fr= (dticks_fr/dtime) * ((2*M_PI)/(this->N * this->T));
      float w_ticks_rl= (dticks_rl/dtime)* ((2*M_PI)/(this->N * this->T));
      float w_ticks_rr= (dticks_rr/dtime) * ((2*M_PI)/(this->N * this->T));
      ...
      ROS_INFO("Ticks: [%f],[%f], [%f]", Vx_ticks,Vy_ticks,Wz_ticks);
